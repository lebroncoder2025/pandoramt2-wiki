  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {classes.map(cls => (
          <div key={cls.name} className={`bg-pandora-card/50 border rounded-lg overflow-hidden transition-all duration-200 ${colorMap[cls.color]}`}>
            <div className="p-5">
              <div className="flex items-center gap-3.5 mb-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-pandora-text/90">{cls.name}</h3>
                  <p className="text-[11px] text-pandora-muted/50">{cls.nameEn}</p>
                </div>
              </div>
                ...
                {cls.subclasses.map(sub => (
                  <div key={sub.name} className="bg-pandora-dark/30 rounded-lg p-3.5 border border-pandora-border/20">
                    <div className="flex items-center justify-between mb-1.5">
                      <h4 className="text-sm font-medium text-pandora-text/80">{sub.name}</h4>
                      <Badge color={badgeColorMap[cls.color]}>{sub.stats}</Badge>
                    </div>
                    <p className="text-[12px] text-pandora-muted/50">{sub.description}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <Card className="mt-6">
        <h3 className="text-sm font-semibold text-pandora-text/80 mb-4">Wskazówki przy wyborze klasy</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[13px] text-pandora-muted/60">
          <div>
            <p className="font-medium text-pandora-text/75 mb-1">PvP (Gracz vs Gracz)</p>
            <p>Wojownik Umysł, Ninja Bliski, Sura Broń Magiczna — najlepsze wybory na PvP.</p>
          </div>
          <div>
            <p className="font-medium text-pandora-text/75 mb-1">PvE (Gracz vs Potwory)</p>
            <p>Szaman Smok, Ninja Daleki, Wojownik Umysł — świetni w farmie i dungeonach.</p>
          </div>
          <div>
            <p className="font-medium text-pandora-text/75 mb-1">Tank / Support</p>
            <p>Wojownik Ciało (tank), Szaman Leczenie (support) — kluczowi w grupach.</p>
          </div>
          <div>
            <p className="font-medium text-pandora-text/75 mb-1">Farm Yang</p>
